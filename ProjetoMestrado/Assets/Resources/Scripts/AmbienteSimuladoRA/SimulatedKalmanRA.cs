...
public class SimulatedKalmanRA : MonoBehaviour
{
    ...
    //outros scripts
    public RobotControlRA robotcontrolRA;

    float deltaT = 0.1f; //passo da operação
    ...
    {
        if (robotcontrolRA.status == true)
        {
            //1 e 2)
            eKant = eK;
            pKant = pK;
            //3)
            eOdon[0] = eKant[0] + robotcontrolRA.forwardSpeed * Mathf.Cos((float)eKant[2]) * deltaT + Random.Range(0.005f, 0.03f);
            eOdon[1] = eKant[1] + robotcontrolRA.forwardSpeed * Mathf.Sin((float)eKant[2]) * deltaT + Random.Range(0.005f, 0.03f);
            eOdon[2] = eKant[2] + robotcontrolRA.angularSpeed * deltaT;

            //4)
            eCam[0] = simulatedVirtualBot.transform.position.x + Random.Range(0.00f, -0.01f); //x da camera
            eCam[1] = simulatedVirtualBot.transform.position.z + Random.Range(0.00f, -0.01f);  //y da camera
